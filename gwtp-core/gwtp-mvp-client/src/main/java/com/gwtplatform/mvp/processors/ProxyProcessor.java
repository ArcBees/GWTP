...
public class ProxyProcessor extends AbstractProcessor {
    ...
    private TypeMirror extractFirstProxyInterface(Element element, List<? extends TypeMirror> interfaces,
        ...
        TypeMirror firstProxyInterface = FluentIterable.from(interfaces)
                .firstMatch(new Predicate<TypeMirror>() {
                    @Override
                    public boolean apply(TypeMirror input) {
                        return asTypeElement(input).getQualifiedName().contentEquals(proxyClass.getCanonicalName());
                    }
                }).orNull();
