package com.gwtplatform.dispatch.server.guice.overridesop;
...
public abstract class AbstractRemoteServiceServlet extends HttpServlet {
     ...
     * escapes the {@link #doPost} method. The default implementation will log
     * the failure and send a generic failure response to the client.
     * <p>
    ...
    protected void doUnexpectedFailure(Throwable e) {
        try {
             ...
             * something has gone wrong is to throw an exception from here. It
             * should be the case that we call the user's implementation code
             * before emitting data into the response, so the only time that
    ...
    /**
     * Returns the strong name of the permutation, as reported by the client
     * that issued the request, or <code>null</code> if it could not be
     * determined. This information is encoded in the
     ...
     * may expose the servlet to some cross-site vulnerabilities. Your
     * implementation should comply with the HTTP/1.1 specification, which
     * includes handling both requests which include a Content-Length header and
     * requests utilizing <code>Transfer-Encoding:
     * chuncked</code>.
     * 
     * @param request
     ...
     * null. This will occur the first time they are accessed after an instance
     * of this class is constructed or deserialized. This method should be
     * called from within a 'synchronized(this) {}' block in order to ensure
     * that only one thread creates the objects.
     */
